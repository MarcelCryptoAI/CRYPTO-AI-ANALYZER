...
/<\/script>/ {
  ...
  print "      case \"ibs_asc\": indicators.sort(function(a, b) { return a.winrate - b.winrate }); break;";
  print "      case \"ibs_desc\": indicators.sort(function(a, b) { return b.winrate - a.winrate }); break;";
  print "      case \"advice_buy_first\": indicators.sort(function(a, b) { return adviceOrder[b.advice] - adviceOrder[a.advice] }); break;";
  print "      case \"advice_sell_first\": indicators.sort(function(a, b) { return adviceOrder[a.advice] - adviceOrder[b.advice] }); break;";
  print "    }";
  print "";
  print "    const container = document.getElementById(\"indicators\");";
  print "    container.innerHTML = \"\";";
  print "";
  print "    indicators.forEach(function(ind) {";
  print "      const overlay = ind.winrate < 0.7 ? \" opacity-overlay\" : \"\";";
  print "      const adviceColor = ind.advice.includes(\"Buy\") ? \"text-emerald-400\" : (ind.advice.includes(\"Sell\") ? \"text-red-400\" : \"text-yellow-300\");";
  ...
  print "  }";
  print "";
  print "  document.querySelectorAll(\".tf-tab\").forEach(function(btn) {";
  print "    btn.addEventListener(\"click\", function() {";
  print "      document.querySelectorAll(\".tf-tab\").forEach(function(b) {";
  print "        b.classList.remove(\"active-tab\", \"text-cyan-400\", \"border-cyan-400\");";
  print "      });";
